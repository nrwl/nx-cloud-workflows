    ...
    clone(message) {
      ...
      for (const member of type.fields.byMember()) {
        ...
        if (member.repeated) {
          ...
          for (const [key2, v2] of Object.entries(source)) {
            copy[key2] = cloneSingularField(v2);
          }
...
var currentBranchKeys = [hashedKey].map((k2) => `${currentBranch}-${k2}`);
var baseBranchKeys = baseBranch ? [hashedKey].map((k2) => `${baseBranch}-${k2}`) : [];
...
`;
    if ((0, import_fs2.existsSync)(process.env.NX_CLOUD_ENV)) {
      (0, import_fs2.appendFileSync)(process.env.NX_CLOUD_ENV, envValue);
    } else {
      (0, import_fs2.writeFileSync)(process.env.NX_CLOUD_ENV, envValue);
    }
  } catch (e) {
    console.log(e);
  }
