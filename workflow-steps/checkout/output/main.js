...
async function main() {
  ...
  if (commitSha.startsWith("origin/")) {
    ...
    if (depth === "0") {
      fetchCommand = 'git fetch --prune --progress --no-recurse-submodules --tags origin "+refs/heads/*:refs/remotes/origin/*" "+refs/pull/*/head:refs/remotes/origin/pr/*/head" "+refs/pull/*/merge:refs/remotes/origin/pr/*/merge"';
    } else {
